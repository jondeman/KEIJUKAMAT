# Prompt Log: image_generation

**Timestamp:** 2026-01-16T21:16:38.615339+00:00
**Category:** ad
**Type:** image_generation

## Metadata

```json
{
  "model": "gemini-3-pro-image-preview",
  "width": 1080,
  "height": 1920,
  "aspect_ratio": "9:16",
  "image_size": "4K"
}
```

## Prompt

```
High-quality vertical infographic one-pager (9:16 aspect ratio) for a Finnish branded content concept pitch.

## OVERALL VISUAL STYLE
Visionary, cinematic, premium Finnish aesthetic. Award-worthy design quality, sophisticated layering, dramatic lighting. Editorial boldness, statement-making, the kind of design that wins at Cannes Lions.
The design must feel premium, professional, and pitch-ready – like it could be presented at a top creative agency.
Mood: visionary, impactful, premium

## LOGO PLACEMENT (CRITICAL)
- TOP LEFT CORNER: Small, elegant Warner Bros. International Television Production Finland logo (use light/white version if dark background)
- TOP RIGHT CORNER: Small, elegant Instrumentarium company logo
- Both logos are placed discretely at the very top edge (approximately 3-4% of image height), maintaining balance and symmetry
- Logos must NOT dominate – they are subtle professional identifiers only

## HEADER SECTION (top 15%)
Below the logos:
- LARGE BOLD TITLE: "Näkövaltaus"
- Subtitle: "Instrumentarium "valtaa" yhden päivän aikana Suomen tunnetuimpia julkisia tiloja ja muuttaa ne he..."
Typography: Bold, modern Finnish advertising style, highly legible, strong visual impact

## MAIN VISUAL SECTION (middle 50%)
Section title: "KONSEPTIN YDIN"

Instrumentarium "valtaa" yhden päivän aikana Suomen tunnetuimpia julkisia tiloja ja muuttaa ne hetkellisesti näkemisen kokemuskeskuksiksi. Helsingin Rautatieaseman odotushalliin ilmestyy jättimäinen näöntarkastuspiste, jossa satunnaiset ohikulkijat voivat testata näkönsä ja saada välittömästi asiantuntija-arvion. Samaan aikaan Oulun torin keskelle pystytetään "maailman suurin silmälasiteline" ja Tampereen Ratinassa järjestetään flash mob, jossa sata optikkoa tekee samanaikaisesti näöntarkastuksi...

This section should visually communicate the core concept in an engaging, infographic style.
Use stylized illustrations, photo compositions, or graphic elements that match the brand aesthetic.
Include key characters, settings, or scenarios that bring the concept to life.
Make it visually rich and immediately understandable.

## PROCESS/FORMAT SECTION (25%)
Title: "NÄIN SE TOIMII"

**Pääformaatti:**
- Yksi koordinoitu "Näkövaltauspäivä" kolmessa kaupungissa samanaikaisesti
- 8-tuntinen live-striimi, joka leikkaa kaupungista toiseen
- Jokainen osallistuja saa QR-koodilla personoidun näköraportin

**Julkaisurytmi:**
- **3 viikkoa ennen:** Mysteerinen teaser-kampanja ("Suomi avaa silmänsä 15.5.")
- **1 viikko ennen:** Vaikuttajat saavat "sokeat laatikot" – avaavat livenä ja paljastavat kutsun
- **D-day:** Live-striimi + reaaliaikainen some-sisältö + PR
- **Jälkeen:** Dokumentti "Suomen näkö" – mitä päivä paljasti suomalaisten silmistä

**Laajennukset:**
- TikTok: Behind-the-scenes rakentamisesta, hauska "reaktio"-sisältö osallistujilta
- Instagram: Stories päivän kulusta, carousel-postaukset näkötilastoista
- OOH: Teaser-mainokset pääkaupungeissa viikkoa ennen
- PR: Embargo-kutsut medioille, data-release päivän tuloksista
- Paid: Geo-targetoitu mainonta tapahtumapäivänä

Present as a clear visual flowchart or step-by-step (3-5 stages) with:
- Each stage in a colored box connected by arrows or visual flow
- Icon or small visual + short descriptive text per stage
- Color-coded using brand palette

## BOTTOM SECTION (10%)
Title: "STRATEGINEN MONIALUSTAINEN ILMIÖ"

Three-column layout showing distribution:
- Primary channel icon + name
- Secondary channels
- Impact/reach indicators

## FOOTER
"Konsepti joka tekee Instrumentariumsta puheenaiheen ja tavoittaa uusia yleisöjä."
A compelling summary of why this concept is perfect for this brand.

## COLOR PALETTE
- Primary brand color: #008D41
- Secondary color: #FFFFFF
- Accent color: #333333
- Background: Rich dark base with premium lighting effects and depth, or striking bold color statement
- Text: High contrast for excellent readability

## TECHNICAL REQUIREMENTS
- Vertical 9:16 aspect ratio (1080x1920 or similar)
- All Finnish text must be clearly readable
- Professional marketing document quality matching top creative agencies
- Avoid generic stock photo aesthetics
- No AI-looking artifacts or distortions in faces
- Clean, sharp typography throughout
- Premium infographic design language
- The one-pager should immediately communicate value and create desire to learn more

```
