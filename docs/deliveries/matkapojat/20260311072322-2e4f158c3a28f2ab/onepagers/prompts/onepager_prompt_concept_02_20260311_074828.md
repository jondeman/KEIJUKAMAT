# One-Pager Prompt — concept_02

## Bussilippu Paratiisiin

```
High-quality horizontal infographic one-pager in 16:9 landscape ratio. Final output should be 3840x2160 (4K UHD) for screen presentation.

## OVERALL VISUAL STYLE
Bold, energetic, contemporary Finnish style. Strong graphics, dynamic composition, social-media-native visual language. Urban, fresh, attention-grabbing. High contrast, unexpected color pops, modern edge.
The design must feel premium, professional, and pitch-ready – like it could be presented at a top creative agency.
Mood: bold, energetic, contemporary

## LOGO PLACEMENT (CRITICAL)
- TOP LEFT CORNER: Small, elegant Warner Bros. International Television Production Finland logo (use light/white version if dark background)
- TOP RIGHT CORNER: Small, elegant Matkapojat company logo
- Both logos are placed discretely at the very top edge (approximately 3-4% of image height), maintaining balance and symmetry
- Logos must NOT dominate – they are subtle professional identifiers only
Jos et löydä tarkkaa logoa tai brändivärejä, käytä Matkapojatn virallista logoa sekä logoon ja brändi-ilmeeseen sopivia värisävyjä.

## HEADER SECTION (top 18%)
Below the logos:
- LARGE BOLD TITLE: "Bussilippu Paratiisiin"
- Subtitle: "Suomalaiset 45–55-vuotiaat ovat sukupolvi, joka on oppinut, että kaikki pitää tehdä itse – lennot..."
Typography: Bold, modern Finnish advertising style, highly legible, strong visual impact

## TIIVISTYS (TOP)
Immediately after the title, include a compact 3-line summary block:
- **MITÄ?** Suomalaiset 45–55-vuotiaat ovat sukupolvi, joka on oppinut, että kaikki pitää tehdä itse – lennot, hotellit, siirtymät, kartat, valuutat. He ovat Booking.com...
- **MITEN?** **Pääformaatti – TikTok-sarja: "Tunnustukset"** - Vertikaalinen, 30–75 sekuntia per video - Rakenne: **3 sekunnin hook → tunnustus → käänne → ratkaisu** - Ho...
- **MIKSI?** Tekee Matkapojatsta puheenaiheen ja muuttaa kiinnostuksen toiminnaksi.
These lines must be punchy and attention-grabbing, capturing the core idea fast.


## LISÄHUOMIOT (kevyt painotus)
Kohderyhmänä oletettavasti +45/55v PKS ulkopuolella
Minkälainen konsepti voisi olla heidän tarjontaa ajatellen (risteilymatkat, kaupunkilomat, kiertomatkat)
Helppous ja ns. ovelta ovelle pitäisi varmaan korostua, kun ajatellaan, mistä kohderyhmä matkustaa?
Käytä tätä ohjaavana sävynä, älä syrjäytä konseptin ydintä.
TÄRKEÄÄ: ÄLÄ kirjoita näitä lisähuomioita sellaisenaan onepageriin.
Tiivistä teksti tai käytä pienempiä fontteja, jotta lauseet eivät katkea '...'.



## MAIN VISUAL SECTION (middle 52%)
Section title: "KONSEPTIN YDIN"

Suomalaiset 45–55-vuotiaat ovat sukupolvi, joka on oppinut, että kaikki pitää tehdä itse – lennot, hotellit, siirtymät, kartat, valuutat. He ovat Booking.comin, Skyscannnerin ja Google Mapsin sukupolvi. He ovat ylpeitä siitä. Ja he ovat uupuneita siitä.

**Bussilippu Paratiisiin** käyttää tätä kulttuurista jännitettä polttoaineenaan: sarja lyhyitä, teräviä TikTok-videoita, joissa oikeat 45–55-vuotiaat suomalaiset "tunnustavat" omatoimimatkailun piilotetun stressin – ja vertaavat sitä siihen, mil...

This section should visually communicate the core concept in a rich, infographic style.
Use stylized illustrations, photo compositions, and graphic elements that match the brand aesthetic.
Include key characters, settings, or scenarios that bring the concept to life.
This must feel like a fully art-directed pitch visual, not a minimal poster.

## PROCESS/FORMAT SECTION (20%)
Title: "NÄIN SE TOIMII"

**Pääformaatti – TikTok-sarja: "Tunnustukset"**
- Vertikaalinen, 30–75 sekuntia per video
- Rakenne: **3 sekunnin hook → tunnustus → käänne → ratkaisu**
- Hook-esimerkkejä:
  - *"Käytin 4 tuntia halvimman lennon etsimiseen. Säästin 23 euroa. Tässä mitä menin tekemään."*
  - *"En ikinä ottaisi valmismatkaa. Sanotaan näin: enää en."*
- Visuaali: matalatuotantoinen look (käsivarakamera, oikeat ihmiset, oikeat paikat) – TikTok-natiivi estetiikka, ei mainos

**Julkaisurytmi ja kaari:**
- **Jakso 1–4 (viikot 1–2):** Ongelmavaihe – pelkkiä tunnustuksia ilman brändiä. Matkapojat ei mainita vielä ollenkaan. Luodaan uteliaisuus.
- **Jakso 5–8 (viikot 3–4):** Käänne – sama henkilö kertoo, mitä tapahtui kun kokeili Matkapoikia. Brändi astuu kuvaan orgaanisesti.
- **Jatkuva UGC-virta:** #OlenTunnustanut -haaste aktivoidaan viikolla 2, sisältöä aggregoidaan brändin tilille

**Laajennukset:**
- **Instagram Reels:** Sama sisältö, hieman viimeistellympi editointi
- **YouTube Shorts:** Pidemmät versiot (90s), enemmän tarinaa
- **Facebook (paid):** Käännevaiheen videot kohdennetusti 45–55v suomalaisille
- **PR:** Tunnustukset-formaatti tarjotaan medialle "ilmiönä" – *Suomalaiset myöntävät: omatoimimatkailu stressaa*
- **OOH:** Bussipysäkeille tunnustus-sitaatit printtinä – *"Säästin 23 euroa. Menin 4 tuntia."* – QR-koodilla TikTok-sarjaan

---

Present as a clear visual flowchart or step-by-step (3-5 stages) with:
- Each stage in a colored box connected by arrows or visual flow
- Icon or small visual + short descriptive text per stage
- Color-coded using brand palette

## BOTTOM SECTION (10%)
Title: "STRATEGINEN MONIALUSTAINEN ILMIÖ"

Three-column layout showing distribution:
- Primary channel icon + name
- Secondary channels
- Impact/reach indicators

## FOOTER
"Konsepti joka tekee Matkapojatsta puheenaiheen ja tavoittaa uusia yleisöjä."
A compelling summary of why this concept is perfect for this brand.

## COLOR PALETTE
- Primary brand color: #3ebfed
- Secondary color: #022860
- Accent color: #00b07b
- Background: Dynamic gradient or bold brand color with graphic elements
- Text: High contrast for excellent readability

## TECHNICAL REQUIREMENTS
- 16:9 landscape layout (target 3840x2160)
- Wide, cinematic composition with clear horizontal flow
- All Finnish text must be clearly readable and well-spaced
- Professional marketing document quality matching top creative agencies
- Avoid generic stock photo aesthetics
- No AI-looking artifacts or distortions in faces
- Clean, sharp typography throughout
- Premium infographic design language
- The one-pager should immediately communicate value and create desire to learn more

```
