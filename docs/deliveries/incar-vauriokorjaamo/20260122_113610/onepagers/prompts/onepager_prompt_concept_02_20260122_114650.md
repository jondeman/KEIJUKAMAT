# One-Pager Prompt — concept_02

## Korjaamomyytinmurtajat

```
High-quality vertical infographic one-pager in A4 portrait ratio (210x297). Final output should be A4-oriented for print-style one-pager layout.

## OVERALL VISUAL STYLE
Bold, energetic, contemporary Finnish style. Strong graphics, dynamic composition, social-media-native visual language. Urban, fresh, attention-grabbing. High contrast, unexpected color pops, modern edge.
The design must feel premium, professional, and pitch-ready – like it could be presented at a top creative agency.
Mood: bold, energetic, contemporary

## LOGO PLACEMENT (CRITICAL)
- TOP LEFT CORNER: Small, elegant Warner Bros. International Television Production Finland logo (use light/white version if dark background)
- TOP RIGHT CORNER: Small, elegant Incar vauriokorjaamo company logo
- Both logos are placed discretely at the very top edge (approximately 3-4% of image height), maintaining balance and symmetry
- Logos must NOT dominate – they are subtle professional identifiers only
Jos et löydä tarkkaa logoa tai brändivärejä, käytä Incar vauriokorjaamon virallista logoa sekä logoon ja brändi-ilmeeseen sopivia värisävyjä.

## HEADER SECTION (top 15%)
Below the logos:
- LARGE BOLD TITLE: "Korjaamomyytinmurtajat"
- Subtitle: "Incarin mekaanikot ja asiakkaat murtavat yhdessä vauriokorjaamoalan yleisimmät myytit ja kauhutar..."
Typography: Bold, modern Finnish advertising style, highly legible, strong visual impact

## TIIVISTYS (TOP)
Immediately after the title, include a compact 3-line summary block:
- **MITÄ?** Incarin mekaanikot ja asiakkaat murtavat yhdessä vauriokorjaamoalan yleisimmät myytit ja kauhutarinat – suoraan, rehellisesti ja välillä brutaalin avoimesti....
- **MITEN?** **Pääformaatti:** - 60–90 sekunnin TikTok-videot - Rakenne: Asiakas lukee myytin/väitteen → Mekaanikko reagoi → Näytetään todisteet (osat, prosessi, dokument...
- **MIKSI?** Tekee Incar vauriokorjaamosta puheenaiheen ja muuttaa kiinnostuksen toiminnaksi.
These lines must be punchy and attention-grabbing, capturing the core idea fast.



## MAIN VISUAL SECTION (middle 50%)
Section title: "KONSEPTIN YDIN"

Incarin mekaanikot ja asiakkaat murtavat yhdessä vauriokorjaamoalan yleisimmät myytit ja kauhutarinat – suoraan, rehellisesti ja välillä brutaalin avoimesti. Formaatissa asiakkaat lukevat ääneen netistä poimittuja pelkoja ja väitteitä ("vakuutusyhtiö pakottaa Incarille", "käyttävät halvimpia osia", "auto ei ole enää koskaan sama"), ja mekaanikko vastaa joko kumoamalla myytin todistein tai myöntämällä "tää pitää joskus paikkansa, mut tässä on syy". Raa'an rehellinen lähestymistapa kääntää luottam...

This section should visually communicate the core concept in a rich, infographic style.
Use stylized illustrations, photo compositions, and graphic elements that match the brand aesthetic.
Include key characters, settings, or scenarios that bring the concept to life.
This must feel like a fully art-directed pitch visual, not a minimal poster.

## PROCESS/FORMAT SECTION (25%)
Title: "NÄIN SE TOIMII"

**Pääformaatti:**
- 60–90 sekunnin TikTok-videot
- Rakenne: Asiakas lukee myytin/väitteen → Mekaanikko reagoi → Näytetään todisteet (osat, prosessi, dokumentti) → Lopputuomio
- Visuaalisesti: splittaus asiakkaan ja mekaanikon välillä, korjaamon "backstage" -materiaali

**Julkaisurytmi:**
- 2 videota/viikko, 8 viikon kausi (16 episodia)
- Teemalliset viikot: "Osien laatu", "Korjausaika", "Sähköautot", "Hinnoittelu"

**Laajennukset:**
- YouTube Shorts ja IG Reels -ristiinjulkaisu
- "Lähetä oma myyttisi" -UGC-keräys TikTokissa (kommentit, duetit)
- PR: Rohkein myönnytys → media-angle ("Incar myönsi ensimmäisenä alalla X:n")
- Mahdollinen pidempi "Myytti vai fakta?" -podcast mekaanikon ja toimittajan kanssa

Present as a clear visual flowchart or step-by-step (3-5 stages) with:
- Each stage in a colored box connected by arrows or visual flow
- Icon or small visual + short descriptive text per stage
- Color-coded using brand palette

## BOTTOM SECTION (10%)
Title: "STRATEGINEN MONIALUSTAINEN ILMIÖ"

Three-column layout showing distribution:
- Primary channel icon + name
- Secondary channels
- Impact/reach indicators

## FOOTER
"Konsepti joka tekee Incar vauriokorjaamosta puheenaiheen ja tavoittaa uusia yleisöjä."
A compelling summary of why this concept is perfect for this brand.

## COLOR PALETTE
- Primary brand color: #0040a0
- Secondary color: #ffffff
- Accent color: #729bda
- Background: Dynamic gradient or bold brand color with graphic elements
- Text: High contrast for excellent readability

## TECHNICAL REQUIREMENTS
- A4 portrait layout (target 2896x4096)
- Dense infographic layout with multiple sections (header + summary + main visual + flow + distribution + footer)
- All Finnish text must be clearly readable and well-spaced
- Professional marketing document quality matching top creative agencies
- Avoid generic stock photo aesthetics
- No AI-looking artifacts or distortions in faces
- Clean, sharp typography throughout
- Premium infographic design language
- The one-pager should immediately communicate value and create desire to learn more

```
